...
public class ExtendedInfluxBackendListenerClient extends AbstractBackendListenerClient implements Runnable {
    private static final Logger log = LoggerFactory.getLogger(ExtendedInfluxBackendListenerClient.class);
    ...
    private ExtendedInfluxdbMetricsSender influxdbMetricsManager;

    private ScheduledExecutorService scheduler;
    private ScheduledFuture<?> timerHandle;

    public ExtendedInfluxBackendListenerClient() {
        super();
    }
        ...
        synchronized (LOCK) {
            for (Map.Entry<String, SamplerMetric> entry : getMetricsInfluxdbPerSampler().entrySet()) {
                ...
                if (entry.getKey().equals(CUMULATED_METRICS)) {
                    ...
                    addMetrics(ExtendedAbstractInfluxdbMetricsSender.tagToStringValue(entry.getKey()), metric);
                }
    ...
    private void addErrorMetric(String transaction, String responseCode, String responseMessage, long count) {
        if (count > 0) {
            ...
            tag.append(TAG_RESPONSE_CODE).append(ExtendedAbstractInfluxdbMetricsSender.tagToStringValue(responseCode));
            tag.append(TAG_RESPONSE_MESSAGE).append(ExtendedAbstractInfluxdbMetricsSender.tagToStringValue(responseMessage));
            tag.append(userTag);
    ...
    public void setupTest(BackendListenerContext context) throws Exception {
        ...
        testTags = ExtendedAbstractInfluxdbMetricsSender.tagToStringValue(context.getParameter("eventTags", ""));
        ...
        for (int i = 0; i < percentilesStringArray.length; i++) {
            if (!StringUtils.isEmpty(percentilesStringArray[i].trim())) {
                try {
                    ...
                    okPercentiles.put(ExtendedAbstractInfluxdbMetricsSender.tagToStringValue(format.format(percentileValue)),
                            percentileValue);
                    koPercentiles.put(ExtendedAbstractInfluxdbMetricsSender.tagToStringValue(format.format(percentileValue)),
                            percentileValue);
                    allPercentiles.put(ExtendedAbstractInfluxdbMetricsSender.tagToStringValue(format.format(percentileValue)),
                            percentileValue);
        ...
        context.getParameterNamesIterator().forEachRemaining(name -> {
            if (StringUtils.isNotBlank(name) && !DEFAULT_ARGS.containsKey(name.trim())
                ...
                userTagBuilder.append(',')
                        .append(ExtendedAbstractInfluxdbMetricsSender
                                .tagToStringValue(tagName))
                        .append('=')
                        .append(ExtendedAbstractInfluxdbMetricsSender.tagToStringValue(
                                tagValue));
        ...
        this.influxdbMetricsManager = (ExtendedInfluxdbMetricsSender) clazz.getDeclaredConstructor().newInstance();
    ...
    private void addAnnotation(boolean startOrEnd) {
        influxdbMetricsManager.addMetric(EVENTS_FOR_ANNOTATION,
                ...
                TEXT +
                        ExtendedAbstractInfluxdbMetricsSender.fieldToStringValue(testTitle +
